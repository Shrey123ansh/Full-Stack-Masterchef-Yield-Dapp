...
import testminttokenAbi from "./testminttoken.json";
import { ethers } from "ethers";

const masterchefAddr = "0x7277B3F59bFA7283Adee067Ee4B76FaDAE2E3772";
...
export const getPoolDetails = async () => {
  ...
  for (let i = 0; i < poolLength; i++) {
    ...
    let userRewardRaw = (
      await masterchef.pendingReward(i, userwallet)
    ).toString();
...
export const mintToken = async (tokenaddress) => {
  let amountToWei = (await convertToWei("1000")).toString();
  let web3connection = await connectWallet();
  ...
  let result = await tokencontract.mint(amountToWei).then(() => {
    return true;
  });
  return result;
};
